...
const fullBodyStyles = ["Đá nét", "Đá tả", "Đâu", "Nhạt", "Linework"];
...
export const tattooDesigns: TattooDesign[] = [
  {
    ...
    name: "Xăm full lưng",
    price: 7000000,
    priceText: "7.000.000đ – 18.000.000đ",
    category: "Full body",
    description:
  ...
  },
  {
    ...
    name: "Xăm full tay",
    price: 6600000,
    priceText: "6.600.000đ – 10.000.000đ",
    category: "Full body",
  ...
  },
  {
    ...
    name: "Xăm full chân",
    price: 9900000,
    priceText: "9.900.000đ – 18.000.000đ",
    category: "Full body",
  ...
  },
  {
    ...
    name: "Xăm full ngực",
    price: 3500000,
    priceText: "3.500.000đ – 7.000.000đ",
    category: "Full body",
  ...
  },
  {
    ...
    duration: "1-6 giờ",
    variants: [
      { position: "Trắng đen / Màu", style: "Mini (<10cm)", sessions: "2-3 giờ", priceSimple: 1000000, priceDifficult: 1500000 },
      { position: "Trắng đen / Màu", style: "A5", sessions: "3-4 giờ", priceSimple: 1500000, priceDifficult: 2000000 },
      { position: "Trắng đen / Màu", style: "A4", sessions: "4-6 giờ", priceSimple: 2000000, priceDifficult: 3000000 },
    ],
...
// Helper: get unique positions from variants
export function getPositions(variants: TattooVariant[]): string[] {
  return [...new Set(variants.map((v) => v.position))];
}

// Helper: get unique styles for a given position
export function getStyles(variants: TattooVariant[], position: string): string[] {
  return [
    ...new Set(
      variants
        .filter((v) => v.position === position && v.style)
        .map((v) => v.style!)
    ),
  ];
}

// Helper: find a specific variant by position and optional style
export function findVariant(
  variants: TattooVariant[],
  position: string,
  style?: string,
): TattooVariant | undefined {
  return variants.find(
    (v) => v.position === position && (style ? v.style === style : true),
  );
}
